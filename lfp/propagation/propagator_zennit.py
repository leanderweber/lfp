        ...
        for key in param_keys:
            if key not in missing:
                ...
                if param is not None:
                    ...
                    setattr(module, key, torch.nn.Parameter(modifier(copy.deepcopy(param.data), key)))
...
class LFPHook(zcore.Hook):
    ...
    def __init__(
        ...
        self.use_input_magnitude = use_input_magnitude
        self.use_param_sign = use_param_sign
        self.is_bn = is_bn
    ...
    def backward(self, module, grad_input, grad_output):
                ...
                for g in grad_output:
                    if g is not None:
                        grad_output_new.append(
                            g / torch.where(g.abs().max() > 0, g.abs().max(), torch.ones_like(g.abs().max())))
                    else:
                        grad_output_new.append(None)
                grad_output = tuple(grad_output_new)
            else:
                if grad_output is not None:
                    grad_output = grad_output / torch.where(grad_output.abs().max() > 0, grad_output.abs().max(),
        ...
        with mod_param_storage_context(module) as param_storing_module:
            for in_mod, param_mod, out_mod in zip(self.input_modifiers, self.param_modifiers, self.output_modifiers):
                ...
                with mod_params(param_storing_module, param_mod, **param_kwargs) as modified, torch.autograd.enable_grad():
            ...
            # Input Relevance
            input_gradients = torch.autograd.grad(outputs, inputs,
            ...
            for key in param_keys:
                ...
                if not self.use_input_magnitude:
                    param_inputs = torch.autograd.grad(outputs, param_datas, grad_outputs=[torch.ones_like(o) for o in outputs], retain_graph=True, create_graph=False)
                param_gradients = torch.autograd.grad(outputs, param_datas,
                                                          grad_outputs=self.gradient_mapper(grad_output[0], outputs),
                                                          retain_graph=True, create_graph=False)
                if not self.use_input_magnitude:
                    param_gradients = [p/torch.where(param_inputs[i] != 0, param_inputs[i], torch.ones_like(param_inputs[i])) for i, p in enumerate(param_gradients)]

                param_relevance = self.reducer(param_datas, param_gradients)

                # LFP
                param_reward = param_relevance
                if not self.use_param_sign:
...
class LFPEpsilon(LFPHook):
    ...
    '''
    def __init__(self, norm_backward, use_input_magnitude, use_param_sign, epsilon=1e-6):
        super().__init__(
            norm_backward=norm_backward,
            use_input_magnitude=use_input_magnitude,
            use_param_sign=use_param_sign,
...
class LFPEpsilonComposite(composites.SpecialFirstNamedLayerMapComposite):
    def __init__(self, norm_backward, use_input_magnitude=True, use_param_sign=False, canonizers=None):
        layer_map = LAYER_MAP_BASE + [
            (ztypes.Linear, LFPEpsilon(norm_backward, use_input_magnitude, use_param_sign)),
            (ztypes.BatchNorm, LFPEpsilon(norm_backward, use_input_magnitude, use_param_sign)),
        ]
        name_map = []
        first_map = []

        super().__init__(layer_map=layer_map, name_map=name_map, first_map=first_map, canonizers=canonizers)
